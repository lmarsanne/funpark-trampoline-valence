...
const faqItems = [
  {
    ...
    answer:
      "Pour valider une formule anniversaire, un minimum de <strong>6 enfants</strong> est requis (l'enfant fêté inclus).",
  },
  {
    question: "Le gâteau et les boissons sont-ils fournis ?",
    answer:
      "OUI, c'est du 100% clé en main ! Nous fournissons le gâteau (Chocolat ou Fruits), les bonbons, les boissons (sirops à volonté) et toute la vaisselle. Aucune nourriture extérieure n'est acceptée (sauf allergie médicale justifiée).",
  },
  {
    question: "Les chaussettes sont-elles vraiment incluses ?",
    answer:
      "Absolument ! Fini les suppléments à l'accueil. Nous fournissons une paire de chaussettes antidérapantes à chaque enfant invité.",
  },
  {
    question: "À partir de quel âge conseillez-vous l'anniversaire ?",
    answer:
      "Nos formules sont idéales à partir de <strong>5 ans</strong>. Les enfants restent sous la surveillance des parents ou accompagnateurs présents (qui ne paient pas l'entrée s'ils ne sautent pas !).",
  },
  {
    question: "Combien de temps faut-il réserver à l'avance ?",
    answer:
      "Les créneaux du week-end partent très vite ! Nous vous conseillons de réserver <strong>3 à 4 semaines à l'avance</strong> pour avoir l'horaire de votre choix.",
  },
  {
    question: "Que se passe-t-il si un invité est absent le jour J ?",
    answer:
      "La réservation est ferme et définitive. <strong>Nous ne procédons à aucun remboursement</strong> en cas d'invité manquant. Le forfait est dû pour le nombre d'enfants réservé initialement.",
  },
  {
    question: "Comment se fait le paiement ?",
    answer:
      "Le paiement s'effectue <strong>en totalité en ligne</strong> au moment de la réservation. Cela valide définitivement votre créneau et vous permet de profiter de la fête sans passer par la caisse à l'arrivée.",
  },
  {
    question: "Faut-il des chaussures ou équipements spéciaux ?",
    answer:
      "Bowling = Vos propres baskets propres (pas de location). Trampoline = Chaussettes antidérapantes (incluses dans les formules !).",
  },
];

const goToReservation = () => {
  window.location.assign("/guidap-reservation-anniv.html");
};
...
const AnniversaireEnfant = () => {
  ...
  return (
    <>
      <Helmet>
        <title>
          Anniversaire Enfant Valence - 100% Fun & 0% Stress | Tout Inclus
        </title>
        <meta
          name="description"
        ...
        <script
          defer
          src="https://valence-trampoline.com/~flock.js"
          data-proxy-url="https://valence-trampoline.com/~api/analytics"
        ></script>
      </Helmet>
          ...
          <div className="w-full md:container md:mx-auto md:max-w-5xl md:px-4 mb-4 md:mb-8">
            <div className="relative md:rounded-3xl overflow-hidden shadow-2xl md:aspect-video">
              ...
              <Button
                ...
                className="absolute bottom-4 right-4 z-10 bg-background/70 backdrop-blur border-border/70 hover:bg-background/85"
              >
                {isMuted ? (
                  <>
                    <Volume2 />
            ...
            <div className="grid grid-cols-1 gap-3 md:gap-6 w-full md:w-[90%] mx-auto">
              {formulasOverview.map((formula, index) => (
                ...
                >
                  {formula.badge && (
                    <div
                      className={`absolute top-0 left-0 right-0 bg-gradient-to-r ${formula.badgeColor} text-white px-4 py-2 text-sm md:text-sm font-bold text-center`}
                    >
                      {formula.badge}
                    </div>
                  )}
                  ...
                  <CardHeader
                    className={`p-4 md:p-6 pb-2 md:pb-2 ${
                      formula.badge ? "pt-12 md:pt-12" : ""
                    }`}
                  >
                  ...
                  <CardContent className="p-4 md:p-6 pt-0 md:pt-0 space-y-2 md:space-y-3">
                    <div className="border-t border-border pt-3 md:pt-4">
                      {formula.features.map((feature, fIndex) => (
                        <div
                          key={fIndex}
                          className="flex items-start gap-2 md:gap-2 mb-2 md:mb-2"
                        >
                          <Check className="w-5 h-5 md:w-5 md:h-5 text-green-500 flex-shrink-0 mt-0.5" />
                          <span className="text-sm md:text-sm text-foreground/80 leading-tight">
                            {feature}
                          </span>
                        </div>
                      ))}
