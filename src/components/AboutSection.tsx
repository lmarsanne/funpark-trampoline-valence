...
const galleryImages = [{
  src: aboutGallery1,
  alt: "Familles sautant sur les trampolines"
}, {
  src: aboutGallery2,
  alt: "Enfants s'amusant dans le Fun Park"
}, {
  src: aboutGallery3,
  alt: "Groupe de jeunes au trampoline park"
}, {
  src: aboutGallery4,
  alt: "Enfants jouant au basket sur trampolines"
}];
const features = [{
  icon: Zap,
              ...
              <CarouselContent>
                {galleryImages.map((image, index) => <CarouselItem key={index}>
                    <div className="relative rounded-2xl md:rounded-3xl overflow-hidden shadow-xl">
                      <img src={image.src} alt={image.alt} className="w-full aspect-[4/5] md:aspect-[3/4] object-cover hover:scale-105 transition-transform duration-500" />
